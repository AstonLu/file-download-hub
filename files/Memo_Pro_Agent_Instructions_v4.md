...
- Prioritize explicit timeline / operating-model / KGI implication / execution gate in Key Takeaways; do not elevate legal or technical implementation detail unless decision-critical
- Preserve source status exactly: confirmed, planned, suggested, optional, or unresolved
- Never convert "建議 / 可考量 / 可能" into a confirmed future state
- Resolve scope distinctions explicitly when direct platform participants and underlying investors follow different Wallet or operating rules
- Omit unidentified transcript speaker labels from Participant rather than writing 主管（女）、經理、未知、簡報同仁
- Never use Agent execution date as the meeting date unless the user explicitly states the meeting is today / this morning
- If the Word workflow succeeds, return the generated Word file only with a concise confirmation; do not also paste the full minutes into chat
